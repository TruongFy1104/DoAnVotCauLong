...
const OrderDetails = () => {
    ...
    const hasSize = details && details.some(item => item.Size && item.Size !== "");
    ...
    return (
        <div className="order-detail-container" style={{
            ...
            <h2 style={{ color: "#2563eb", marginBottom: 24 }}>Chi tiết đơn hàng #{orderInfo?.OrderId}</h2>
            <div style={{
                marginBottom: 24,
                background: "#f6f8fa",
                padding: 16,
                ...
                <div><b>Họ tên:</b> {customer?.Firstname} {customer?.Lastname}</div>
                <div><b>Địa chỉ:</b> {customer?.Address}</div>
                <div><b>Số điện thoại:</b> {customer?.Mobile}</div>
            ...
            <div style={{
                ...
                <div><b>Ngày đặt:</b> {orderInfo?.CreateAt ? new Date(orderInfo.CreateAt).toLocaleString() : ""}</div>
                <div><b>Phương thức thanh toán:</b> {orderInfo?.PaymentMethod === "cod" ? "Thanh toán khi nhận hàng" : "Chuyển khoản"}</div>
                <div>
                  <b>Trạng thái:</b>{" "}
                    ...
                    <table style={{
                        ...
                        <thead>
                            <tr style={{ background: "#e5e7eb" }}>
                                ...
                                <th style={{ padding: 10, border: "1px solid #ddd" }}>Tên sản phẩm</th>
                                {hasSize && <th style={{ padding: 10, border: "1px solid #ddd" }}>Size</th>}
                                <th style={{ padding: 10, border: "1px solid #ddd" }}>Số lượng</th>
                                <th style={{ padding: 10, border: "1px solid #ddd" }}>Giá</th>
                                <th style={{ padding: 10, border: "1px solid #ddd" }}>Thành tiền</th>
                            </tr>
                        </thead>
                        <tbody>
                            {details && details.length > 0 ? details.map((item, idx) => {
                                ...
                                const imageUrl = item.ImageUrl || (product.Avatar ? `http://localhost:3000/uploads/${product.Avatar}` : "");
                                const productName = item.ProductName || product.ProductName || "";
                                ...
                                return (
                                    <tr key={idx}>
                                        ...
                                        {hasSize && <td style={{ padding: 10, border: "1px solid #ddd" }}>{item.Size || "-"}</td>}
                                        <td style={{ padding: 10, border: "1px solid #ddd" }}>{quantity}</td>
                                        <td style={{ padding: 10, border: "1px solid #ddd" }}>
                                            {Number(price).toLocaleString()} VND
                                        </td>
                                        <td style={{ padding: 10, border: "1px solid #ddd" }}>
                                            {(Number(price) * Number(quantity)).toLocaleString()} VND
                                        </td>
                                    </tr>
                                );
                            }) : (
                                <tr>
                                    <td colSpan={hasSize ? 6 : 5} style={{ textAlign: "center", padding: 16 }}>Không có sản phẩm nào.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
            ...
            <div style={{
                ...
                <div><b>Tạm tính:</b> {formatPrice(orderInfo?.SubPrice)}</div>
                <div><b>Giảm giá:</b> {formatPrice(orderInfo?.Discount)}</div>
