...
const AddProductForm = () => {
  ...
  const [clothingSizes, setClothingSizes] = useState([{ size: '', quantity: 0 }]);
      ...
      const fetchProduct = async () => {
        try {
          ...
          if (product.ClothingSizes && Array.isArray(product.ClothingSizes)) {
            setClothingSizes(product.ClothingSizes.map(item => ({
              size: item.Size,
          ...
          if (product.ShoeSizes && Array.isArray(product.ShoeSizes)) {
            setShoeSizes(product.ShoeSizes.map(item => ({
              size: item.Size,
              quantity: item.Quantity
            })));
          } else {
  ...
  const handleSubmit = async (event) => {
    ...
    console.log("CategoryId FE gửi lên:", productCategory, typeof productCategory);
    ...
    if (categories.find(cat => cat.CategoryId == productCategory)?.CategoryName === 'Quần áo') {
      formData.append('clothingSizes', JSON.stringify(clothingSizes.filter(s => s.size)));
    }
    if (categories.find(cat => cat.CategoryId == productCategory)?.CategoryName === 'Giày') {
      formData.append('shoeSizes', JSON.stringify(shoeSizes.filter(s => s.size)));
    }
          ...
          <div className="card w-100">
            <div className="card-body p-4">
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-3">
                  ...
                  {productImage && (
                    <div className="mt-3">
                      <img
                        id="imagePreview"
                        src={typeof productImage === 'string' ? `http://localhost:3000/uploads/${productImage}` : URL.createObjectURL(productImage)}
                 ...
                 <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                    {productMultiImage &&
                      productMultiImage.map((image, index) => (
                        ...
                        >
                          <img
                            src={typeof image === 'string' ? `http://localhost:3000/uploads/${image}` : URL.createObjectURL(image)}
                ...
                <div className="mb-3">
                  <label htmlFor="productCategory" className="form-label fw-600">Danh mục</label>
                  <select
                    name='CategoryId'
                    className="form-select"
                    id="productCategory"
                ...
                {/* Size cho quần áo */}
                {productCategory && categories.find(cat => cat.CategoryId == productCategory)?.CategoryName === 'Quần áo' && (
                  <div className="mb-3">
                      ...
                      <div key={idx} className="row mb-2">
                        ...
                        <div className="col-md-2">
                          <button type="button" className="btn btn-danger btn-sm" onClick={() => {
                            setClothingSizes(clothingSizes.filter((_, i) => i !== idx));
                          }}>Xóa</button>
                        </div>
                      </div>
                    ))}
                    <button type="button" className="btn btn-success btn-sm" onClick={() => setClothingSizes([...clothingSizes, { size: '', quantity: 0 }])}>Thêm size</button>
                  </div>
                )}

                {/* Size cho giày */}
                {productCategory && categories.find(cat => cat.CategoryId == productCategory)?.CategoryName === 'Giày' && (
                  <div className="mb-3">
                    ...
                    {shoeSizes.map((item, idx) => (
                      <div key={idx} className="row mb-2">
                        ...
                        <div className="col-md-2">
                          <button type="button" className="btn btn-danger btn-sm" onClick={() => {
                            setShoeSizes(shoeSizes.filter((_, i) => i !== idx));
                          }}>Xóa</button>
                        </div>
                      </div>
                    ))}
                    <button type="button" className="btn btn-success btn-sm" onClick={() => setShoeSizes([...shoeSizes, { size: '', quantity: 0 }])}>Thêm size</button>
                  </div>
                )}
