...
export default function ProductDetail() {
  ...
  const fetchReplies = async (commentId) => {
    try {
      const res = await fetch(`http://localhost:3000/comments/${commentId}/replies`);
  ...
  const handleAddToCart = () => {
    ...
    if (
      (Array.isArray(productDetails.ClothingSizes) && productDetails.ClothingSizes.length > 0 ||
       Array.isArray(productDetails.ShoeSizes) && productDetails.ShoeSizes.length > 0)
      && !selectedSize
    ) {
  ...
  return (
    <div className="pd-container pd-large">
      ...
      <div className="pd-breadcrumb">
        ...
        <Link to="/home" className="pd-breadcrumb-link">Chi tiết sản phẩm</Link>
      </div>
      <div className="pd-main pd-main-large">
                ...
                {productDetails?.Avatar && (
                  <div className="col-3">
                    <div className="small-image-wrapper">
                      <img
                        src={`http://localhost:3000/uploads/${productDetails.Avatar}`}
                        alt="Avatar"
                        className={`img-fluid${largeImage.endsWith(productDetails.Avatar) ? " active" : ""}`}
                        onClick={() => setLargeImage(`http://localhost:3000/uploads/${productDetails.Avatar}`)}
                      />
                    </div>
                  </div>
                )}
                  ...
                  <div className="col-3" key={index}>
                    <div className="small-image-wrapper">
                      <img
                        ...
                        className={`img-fluid${largeImage.endsWith(image.ImageUrl) ? " active" : ""}`}
        ...
        <div className="pd-info pd-info-large">
          ...
          <div className="pd-info-extra">
            <span className="pd-info-brand">Thương hiệu: <b>{productDetails.Brand?.BrandName}</b></span>
            <span className="pd-info-status">Tình trạng: <b>Còn hàng</b></span>
            <span className="pd-info-sku">Mã SP: <b>{productDetails.ProductId}</b></span>
          </div>
          <div className="pd-extra-info" style={{marginBottom: 0}}>
            <div className="pd-discount">Mua online tặng ngay quấn cán Yonex</div>
          </div>
          <div className="pd-price">{Number(productDetails.Price).toLocaleString()}₫</div>
            ...
            productDetails.ClothingSizes.length > 0 && (
              <div className="pd-size-list">
                ...
                {productDetails.ClothingSizes.map((item, idx) => (
                  <span
                    key={idx}
                    className={`pd-size-item${selectedSize === item.Size ? " selected" : ""}`}
        ...
        <div className="pd-specs-table pd-specs-table-vertical">
          {specifications.map((spec, idx) => (
            <div className="pd-spec-row" key={idx}>
              <div className="pd-spec-cell pd-spec-label">{spec.SpecificationName}</div>
      ...
      <div className="pd-comments-section pd-comments-fullwidth">
        ...
        <div className="pd-comments-list pd-comments-list-full">
          {Array.isArray(comments) && comments.length > 0 ? (
            comments.map((c) => (
              <div key={c.CommentId} className="pd-comment-item pd-comment-item-full">
                ...
                {/* Nút xem phản hồi */}
                {Array.isArray(replies[c.CommentId]) && replies[c.CommentId].length > 0 && (
                  <button
                    className="pd-toggle-replies-btn"
                    style={{ marginLeft: 8, color: "#007bff", background: "none", border: "none", cursor: "pointer" }}
                    onClick={() => {
                      if (!openReplies[c.CommentId]) fetchReplies(c.CommentId);
                      setOpenReplies(prev => ({
                        ...prev,
                  ...
                  >
                    {openReplies[c.CommentId] ? "Ẩn" : "Xem"} {replies[c.CommentId].length} phản hồi
                  </button>
                )}

                {/* Hiển thị replies nếu đang mở */}
                {openReplies[c.CommentId] && Array.isArray(replies[c.CommentId]) && replies[c.CommentId].length > 0 && (
                ...
                {replyingTo === c.CommentId && (
                  <div className="pd-reply-box">
                    <input
                      ...
                      onChange={e =>
                        setReplyInput({ ...replyInput, [c.CommentId]: e.target.value })
                      }
                      style={{ width: "70%", marginRight: 8 }}
                    />
                    <button onClick={() => handleReply(c.CommentId)}>Gửi</button>
                  </div>
                )}
              </div>
            ))
